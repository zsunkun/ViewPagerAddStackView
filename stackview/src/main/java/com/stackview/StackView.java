...
public class StackView extends AdapterViewAnimator {
     ...
     */
    private static final float PERSPECTIVE_SHIFT_FACTOR_Y = 0.00f;
...
//    private ImageView mClickFeedback;
    ...
    public StackView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.StackView, defStyleAttr, 0);

        mResOutColor = a.getColor(
                R.styleable.StackView_resOutColor, 0);
        mClickColor = a.getColor(
                R.styleable.StackView_clickColor, 0);
...
//        }
    ...
    private float[] alphas = new float[]{1.0f, 0.8f, 0.6f, 0.4f};
    ...
    private void transformViewAtIndex(int index, final View view, boolean animate) {
        ...
        final float transY = (perspectiveTranslationY + scaleShiftCorrectionY) * 0.2f;
        ...
        float alpha = alphas[mMaxNumActiveViews - 2 - index];
        ...
        if (animate) {
            ...
            PropertyValuesHolder alphaProp = PropertyValuesHolder.ofFloat("alpha", alpha);

            ObjectAnimator oa = ObjectAnimator.ofPropertyValuesHolder(view, scalePropX, scalePropY,
                    translationY, translationX, alphaProp);
...
//                beginGestureIfNeeded(deltaX);
                if ((int) Math.abs(deltaX) > mTouchSlop && mSwipeGestureType == GESTURE_NONE) {
                    return true;
                }
                break;
            }
